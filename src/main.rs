use strata_kill_test::cli::{self, CliCommand};
...
fn main() {
    ...
    // Process logging flags before command
    let mut command_args: Vec<String> = Vec::new();
    for arg in args.iter().skip(1) {
        match arg.as_str() {
            "--debug" => strata_kill_test::cognition::logging::set_log_level(
                strata_kill_test::cognition::logging::LogLevel::Debug,
            ),
            "--trace" => strata_kill_test::cognition::logging::set_log_level(
                strata_kill_test::cognition::logging::LogLevel::Trace,
            ),
            "--minimal-logs" => strata_kill_test::cognition::logging::set_log_level(
                strata_kill_test::cognition::logging::LogLevel::Error,
            ),
    ...
    if !command_args.is_empty() {
        match cli::commands::parse_command(&command_args) {
            Ok(command) => {
                match command {
                    CliCommand::Run(input) => {
                        let output = cli::runner::CliRunner::run(&input);
                        println!("{}", cli::output::format_output(&output));
                    }
                    CliCommand::Replay(path) => {
                        ...
                        let trace: strata_kill_test::cognition::trace::types::TraceRecord = match serde_json::from_str(&content) {
                            Ok(t) => t,
                            Err(e) => {
                        ...
                        let (_output, matches) = cli::runner::CliRunner::replay(&trace);
                        if matches {
                            println!("REPLAY PASS: output matches original trace");
                        } else {
                            println!("REPLAY FAIL: output diverges from original trace");
                        }
                    }
                    CliCommand::Inspect(trace_id) => {
                        let path = format!("trace-{}.json", trace_id);
                        match std::fs::read_to_string(&path) {
                            Ok(content) => {
                                match serde_json::from_str::<strata_kill_test::cognition::trace::types::TraceRecord>(&content) {
                                    Ok(trace) => println!("{}", cli::output::format_trace(&trace)),
                                    Err(e) => eprintln!("Error parsing trace file: {}", e),
                                }
                            }
                            Err(_) => {
                                eprintln!("Trace not found: {} (looked for {})", trace_id, path);
                            }
                        }
                    }
                    CliCommand::Verify => {
                        let report = cli::runner::CliRunner::verify();
                        println!("{}", cli::output::format_verification(&report));
                    }
                    CliCommand::Goals => {
                        let output = cli::runner::CliRunner::run("");
                        println!("{}", cli::output::format_goals(&output.goal_evaluations, &output.updated_goal_state));
                    }
                    CliCommand::Memory => {
                        let output = cli::runner::CliRunner::run("");
                        println!("{}", cli::output::format_memory(&output.memory_snapshot));
                    }
                }
            }
            Err(e) => {
        ...
        match command {
            ...
            "test_e6" => {
                ...
                let seq_events: Vec<SequencedEvent> = events.iter().enumerate()
                    .map(|(i, e)| SequencedEvent::from_seq(i as u64, e.clone()))
                    .collect();
