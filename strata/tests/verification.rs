...
use strata::{
    detect_causal_cycle, log_hash, project_default, replay, state_hash, Event, EventType, Kernel,
    KernelError,
};
...
fn check_replay_equivalence(events: &[Event], n: usize) -> Result<(), String> {
    ...
    let kernel = Kernel::new_test(events.to_vec());
    let kernel_hash = format!("{:?}", kernel.get_state());
...
fn check_cycle_rejection_completeness(
    ...
    let mut kernel = Kernel::new_test(vec![]);
    let mut committed_valid: Vec<Event> = Vec::new();
    for ev in valid_events {
        match kernel.commit(ev.clone()) {
...
/// P5 — Projection Isolation: G₁ access does not affect G₀ replay or kernel state.
fn check_projection_isolation(events: &[Event]) -> Result<(), String> {
    ...
    let kernel = Kernel::new_test(events.to_vec());
    let _ = kernel.get_causal_graph();
    let kh = format!("{:?}", kernel.get_state());
    if kh != before {
        return Err(format!("P5 FAILED: kernel state changed after G₁ access\n  replay: {}\n  kernel: {}", before, kh));
    }
    Ok(())
}
...
fn check_trace_determinism(events: &[Event]) -> Result<(), String> {
    ...
    for ev in events {
        ...
        let kernel = Kernel::new_test(events.to_vec());
        let g1k = kernel.get_causal_graph();
        for _ in 0..5 { traces.push(format!("{:?}", g1k.trace_causal_chain(&ev.id, max_hops))); }
...
fn replay_hash_equivalence(events: &[Event]) -> Result<(), String> {
    ...
    let live_hash = state_hash(kernel.get_state());
...
/// Helper: commit a sequence of events, expecting each to succeed.
fn commit_sequence(kernel: &mut Kernel, events: &[Event]) -> Result<(), String> {
    for e in events {
        kernel.commit(e.clone()).map_err(|err| {
            format!("unexpected commit failure for '{}': {}", e.id, err)
        })?;
    }
    Ok(())
}

/// Helper: attempt a commit that should fail, verify it fails, and confirm
/// the kernel state is unchanged from `prior_hash`.
fn expect_commit_failure(
    kernel: &mut Kernel,
    event: &Event,
    prior_hash: &str,
) -> Result<(), String> {
    let result = kernel.commit(event.clone());
    if result.is_ok() {
        return Err(format!("commit of '{}' should have failed", event.id));
    }
    let current_hash = state_hash(kernel.get_state()).to_string();
    if current_hash != *prior_hash {
        return Err(format!(
            "state mutated after failed commit of '{}': hash changed from {} to {}",
...
fn corruption_duplicate_event_id_is_accepted_by_commit() {
    ...
    kernel.commit(e1).expect("first commit must succeed");
    kernel.commit(e2).expect("second commit with same event ID must succeed (kernel allows)");
    assert_eq!(kernel.get_state().node_count(), 2, "both nodes must exist");
}
...
fn corruption_timestamp_regression() {
    ...
    kernel.commit(e2).expect("second commit must succeed even with lower ts");
...
fn corruption_orphan_cause_accepted_by_commit() {
    ...
    kernel.commit(e1).expect("first commit must succeed");
    kernel.commit(e2).expect("commit with orphan cause must succeed (kernel allows)");
...
fn corruption_empty_node_id() {
    ...
    let prior = state_hash(kernel.get_state()).to_string();
