...
use strata::api::command::{diff, Command, CommandExecutor, CommandResult};
...
fn equivalence_ingest() {
    ...
    let mut eng = StrataEngine::from_events(vec![]);
    let direct = eng.ingest_event(event.clone());

    let mut eng2 = StrataEngine::from_events(vec![]);
...
fn equivalence_validate() {
    ...
    let engine = StrataEngine::from_events(vec![]);
    let direct_ok = engine.validate(&event);
    let direct_bad = engine.validate(&bad_event);

    let mut exec_engine = StrataEngine::from_events(vec![]);
