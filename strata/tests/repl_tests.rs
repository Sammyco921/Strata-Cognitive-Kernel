...
fn repl_execute(bootstrap: &mut TestBootstrap, session: &mut SessionManager, cmd: CliCommand) -> ResultPayload {
...
fn batch_vs_repl_equivalence_query_state() {
    let mut bs_batch = test_bootstrap(vec![]);
    let mut bs_repl = test_bootstrap(vec![]);
...
fn session_id_does_not_affect_execution_result() {
    let mut bs_a = test_bootstrap(vec![]);
    let mut bs_b = test_bootstrap(vec![]);
...
fn repeated_sessions_same_trace_sequence() {
    let mut bs1 = test_bootstrap(vec![]);
    let mut bs2 = test_bootstrap(vec![]);
