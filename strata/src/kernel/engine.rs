...
use crate::kernel::{EventPersister, NullPersister};
use crate::persistence::ProductionPersister;

/// Configuration for production kernel initialisation (no configurable params yet).
pub struct KernelConfig;

/// Configuration for test kernel initialisation from an explicit event list.
pub struct TestConfig {
    pub seed_events: Vec<Event>,
}

/// Private unified initialisation pipeline.
///
/// Every kernel — production or test — is constructed through this function.
fn init_kernel<P: EventPersister>(persister: P, seed_events: Option<Vec<Event>>) -> Kernel<P> {
    eprintln!("[strata] initializing kernel...");

    let (state, clock, prior_events) = match seed_events {
        Some(events) => {
        ...
        None => {
                let all_events = persister.load_events();
                let snap = persister.load_snapshot();

                match snap {
                    Some((snap_state, snap_ts)) => {
                        ...
                        if !remaining.is_empty() {
                            ...
                            (snap_state, snap_ts, all_events)
                        }
                    }
                    None => {
                        if !all_events.is_empty() {
                            eprintln!("[strata] replaying {} events from log...", all_events.len());
                        }
                        let state = replay(&all_events);
                        let clock = all_events.last().map(|e| e.timestamp).unwrap_or(0);
                        (state, clock, all_events)
                    }
                }
        ...
        Kernel { persister, state, clock, event_count: clock, prior_events }
}

/// G₀ = Ground Truth Event Graph (events + immutable state replay).
pub struct Kernel<P: EventPersister> {
...
impl<P: EventPersister> Kernel<P> {
    /// Create a kernel from an already-constructed persister (internal helper).
    pub(crate) fn new(persister: P) -> Self {
        init_kernel(persister, None)
    }

    /// Create a kernel with a specific event seed (internal helper).
    pub(crate) fn with_events(persister: P, events: Vec<Event>) -> Self {
        init_kernel(persister, Some(events))
    }
    ...
    pub fn prior_events(&self) -> &[Event] {
        &self.prior_events
    }

    pub fn save_snapshot(&self) -> Result<(), KernelError> {
        self.persister.save_snapshot(&self.state, self.clock)?;
...
impl Kernel<ProductionPersister> {
    /// Create a production kernel from configuration (loads persisted state).
    pub fn production(_config: KernelConfig) -> Self {
        init_kernel(ProductionPersister, None)
    }
}

impl Kernel<NullPersister> {
    /// Create a test kernel from an explicit seed event list (no I/O).
    pub fn test(config: TestConfig) -> Self {
        init_kernel(NullPersister, Some(config.seed_events))
    }
}
...
mod tests {
    ...
    fn test_create_node_and_edge() {
        let mut k = Kernel::test(TestConfig { seed_events: vec![] });
    ...
    fn test_missing_node_ref_rejected() {
        let k = Kernel::test(TestConfig { seed_events: vec![] });
    ...
    fn tb61_truth_isolation() {
        ...
        let mut state_g0_after = replay(&events);
        assert_eq!(state_g0, state_g0_after, "T-B6.1: G₀ replay must be invariant to G₁ modification");

        // Just verify that replay is deterministic — no G₁ dependency in kernel.
        let state_via_kernel = replay(&events);
        assert_eq!(state_g0, state_via_kernel, "T-B6.1: G₀ replay must be deterministic");
    }
    ...
    fn tb63_explanation_anchoring() {
        ...
        assert_eq!(
            g1.event_nodes.len(),
            3,
            "T-B6.3: G₁ should have 3 event nodes from 3 G₀ events"
        );
    }
    ...
    fn tb72_snapshot_equivalence() {
        ...
        let kernel_state = {
            let k = Kernel::test(TestConfig { seed_events: events.clone() });
            k.state.clone()
        };
