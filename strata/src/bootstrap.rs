...
use crate::api::command::{Command, CommandClass, CommandExecutor, CommandResult, EnvelopeExecutor};
...
pub struct Bootstrap<P: EventPersister> {
    engine: StrataEngine<P>,
    event_counter: u64,
    trace_enabled: bool,
    /// Deterministic audit log (observational only).  Records one entry
    /// per executed command after the result is produced.
    pub audit_log: AuditLog,
}

impl Bootstrap<ProductionPersister> {
    ...
    pub fn new_with_trace(trace_enabled: bool) -> Self {
        let kernel = Kernel::production(KernelConfig);
...
impl<P: EventPersister> Bootstrap<P> {

    /// Create a Bootstrap from pre-constructed parts (internal).
    pub(crate) fn from_parts(
        ...
        Bootstrap { engine, event_counter, trace_enabled, audit_log }
    }
    ...
    pub fn execute(&mut self, envelope: CommandEnvelope) -> CommandResultV1 {
        ...
        let result = CommandResultV1::new(trace_id, class, payload);
        // ── Audit (E4: ordering matches execution — append after result) ─
        self.audit_log.append(trace_id, class, command_name, result.is_ok());
        result
    }
    ...
    #[doc(hidden)]
    pub fn engine(&self) -> &StrataEngine<P> {
        &self.engine
    }
...
impl<P: EventPersister> EnvelopeExecutor for Bootstrap<P> {
