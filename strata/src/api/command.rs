...
/// Trait for executing a fully-packaged `CommandEnvelope` and returning a
/// `CommandResultV1`.  This is the *only* dependency that `ApiDispatcher`
/// needs from the bootstrap layer, breaking the circular dependency between
/// `api` and `bootstrap`.
pub trait EnvelopeExecutor {
...
impl Command {
    ...
    /// Return a stable variant name for audit logging.
    pub fn name(&self) -> &'static str {
        match self {
            Command::Validate(_) => "Validate",
            Command::Ingest(_) => "Ingest",
            Command::Replay(_) => "Replay",
            Command::QueryState => "QueryState",
            Command::Explain { .. } => "Explain",
            Command::CausalChain(_) => "CausalChain",
            Command::ExportSnapshot => "ExportSnapshot",
            Command::GetNode(_) => "GetNode",
            Command::GetEdge(_) => "GetEdge",
            Command::ListNodes => "ListNodes",
            Command::ListEdges => "ListEdges",
            Command::EventById(_) => "EventById",
            Command::EventsForNode(_) => "EventsForNode",
            Command::EventsBetween { .. } => "EventsBetween",
            Command::LatestEvents(_) => "LatestEvents",
            Command::SnapshotMetadata => "SnapshotMetadata",
            Command::GetVersion => "GetVersion",
            Command::GetSchemaVersion => "GetSchemaVersion",
            Command::ValidateLog => "ValidateLog",
            Command::ReplayCheck => "ReplayCheck",
            Command::WorkflowList => "WorkflowList",
            Command::WorkflowRun(_) => "WorkflowRun",
            Command::WorkflowValidate => "WorkflowValidate",
