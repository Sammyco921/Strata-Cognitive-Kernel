...
use crate::api::command::EnvelopeExecutor;
...
pub struct ApiDispatcher {
    executor: Box<dyn EnvelopeExecutor>,
}

impl ApiDispatcher {
    /// Create a new dispatcher backed by the given executor.
    pub fn new(executor: Box<dyn EnvelopeExecutor>) -> Self {
        ApiDispatcher { executor }
    }

    /// Create a dispatcher from an explicit event list (test helper, not
    /// available in release builds to avoid bootstrap circular dependency).
    #[cfg(any(test, debug_assertions))]
    pub fn from_events(events: Vec<Event>) -> Self {
        let kernel = Kernel::test(TestConfig { seed_events: events });
        let engine = crate::api::StrataEngine::from_kernel(kernel);
        let executor = Box::new(Bootstrap::from_parts(engine, 0, false, AuditLog::new()));
        ApiDispatcher { executor }
    }
...
impl Default for ApiDispatcher {
    fn default() -> Self {
        panic!("ApiDispatcher::default() requires an executor. Use ApiDispatcher::new(executor).")
    }
}
