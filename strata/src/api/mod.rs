...
use crate::projection::causal::{project_default, CausalChainLink};
...
/// ```
pub struct StrataEngine<P: EventPersister> {
    kernel: Kernel<P>,
}

impl<P: EventPersister> StrataEngine<P> {
    /// Initialise the engine with a specific persistence backend.
    pub(crate) fn new(persister: P) -> Self {
        StrataEngine { kernel: Kernel::new(persister) }
    }

    /// Create an engine wrapping an already-constructed kernel (internal).
    pub(crate) fn from_kernel(kernel: Kernel<P>) -> Self {
        StrataEngine { kernel }
    }
}

impl<P: EventPersister> Engine for StrataEngine<P> {
    // ── Core Operations ─────────────────────────────────────────────────────
    ...
    fn get_explanation(&self, node_id: &str, property_key: Option<&str>) -> ExplanationView {
        let g1 = project_default(self.kernel.prior_events());
        let ex = g1.explain_belief(self.kernel.get_state(), node_id, property_key, 100);
        ExplanationView::from_explanation(&ex)
    }

    fn export_snapshot(&self) -> Result<String, KernelError> {
        self.kernel.save_snapshot()?;
        let snap = persistence::Snapshot::new(
            self.kernel.get_state().clone(),
    ...
    fn causal_chain(&self, event_id: &str) -> Vec<CausalChainLink> {
        let g1 = project_default(self.kernel.prior_events());
        g1.trace_causal_chain(event_id, 100)
    }
