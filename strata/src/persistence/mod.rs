...
/// Production implementation of `EventPersister` backed by filesystem I/O.
pub struct ProductionPersister;

impl EventPersister for ProductionPersister {
    fn append_event(&mut self, event: &Event) -> Result<(), KernelError> {
        append_event(event)
    }

    fn load_events(&self) -> Vec<Event> {
        load_all_events().unwrap_or_default()
    }

    fn load_snapshot(&self) -> Option<(GraphState, u64)> {
        match load_snapshot() {
            Ok(Some((snap, ts))) => Some((snap.state, ts)),
            _ => None,
        }
    }

    fn save_snapshot(&self, state: &GraphState, clock: u64) -> Result<(), KernelError> {
        save_snapshot(state, clock)
    }
}
